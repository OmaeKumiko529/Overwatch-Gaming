...
该模板支持完整的帖子线程模型、JWT 认证、通知系统和后台管理面板，可直接用于二次开发或学习全栈架构。，可用于学习用途分发。此项目使用 MIT 许可证开源。public 目录下包含的守望先锋相关美术资源（英雄职责图标）来自第三方的版权财产，仅供演示用途，不适用 MIT 许可证，商业使用或再分发需要用自有素材替换。
...
CREATE TABLE users (
  ...
  is_admin      INTEGER NOT NULL DEFAULT 0,             -- 遗存字段，兼容旧数据
  uid           TEXT UNIQUE,                            -- 如 u-20260101-0000
  userrank      INTEGER NOT NULL DEFAULT 0,             -- 0=访客 1=玩家 2=管理员 3=超级管理员
...
CREATE TABLE posts (
  ...
  category   TEXT NOT NULL DEFAULT 'general',            -- 或 'comment' 表示子帖
  likes      INTEGER NOT NULL DEFAULT 0,
  context    TEXT DEFAULT '#',                           -- 嵌套路径，如 #/p-xxx/u-yyy
  parent_id  INTEGER,                                   -- NULL 表示主帖
  pid        TEXT UNIQUE,                                -- 如 p-20260105-a1b2
  postrank   TEXT NOT NULL DEFAULT '69',                 -- FF=红 69=普通 78=绿 00=黑
...
CREATE TABLE teams (
  id           INTEGER PRIMARY KEY AUTOINCREMENT,
  name         TEXT NOT NULL UNIQUE,         -- 原始名+随机后缀
...
CREATE TABLE notifications (
  id         INTEGER PRIMARY KEY AUTOINCREMENT,
  type       TEXT NOT NULL,          -- 'mention' / 'comment' / 'like'
  author     TEXT,                   -- 触发者 UID
  root       TEXT,                   -- 关联帖子 PID
  to_user    INTEGER NOT NULL,       -- 接收者用户 ID
...
CREATE TABLE ow_heroes (
  id          INTEGER PRIMARY KEY AUTOINCREMENT,
  hero_key    TEXT NOT NULL UNIQUE,            -- 如 genji
  name        TEXT NOT NULL,                   -- 如 源氏
  portrait    TEXT,                            -- 头像 URL
  role        TEXT NOT NULL,                   -- tank / damage / support
  subrole     TEXT,                            -- 细分职责
  data_json   TEXT NOT NULL,                   -- 完整英雄详情（JSON）
...
以上功能较好的规避了原版SQLite的部分问题，能够支撑更大的数据库读写操作。
...
| DELETE | /api/posts/:pid | 必需 | 删除帖子及所有子帖（仅帖主，通过 context LIKE 匹配） |
...
| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| GET | /api/heroes | 无 | 获取英雄列表，支持 role 筛选 (tank/damage/support) |
| GET | /api/heroes/:key | 无 | 获取单个英雄详情（含完整 data_json） |
| POST | /api/heroes/sync | 必需(Admin) | 从 OverFast API 同步所有英雄数据 |
...
通过上下文关系，直接写入PID，能够有效减轻SQL中LIKE的压力，在子帖数量庞大的情况下能快速反应。
...
实际上，点赞是一个很不好写的操作，尤其是多平台、高频的点赞，对前端处理能力和SQL库的读写能力要求很高。本项目使用异步处理机制，能有效保证高压力下结果的正确输出。
...
这算是一个简单的前端的SQL可视化读写界面了，如果你需要，也可以把它单独拆走，这对于我个人的诸多项目非常实用。
...
users 和 posts 表的 initSchema 函数在创建表后通过 PRAGMA table_info 检查列的存量，对缺失的列执行 ALTER TABLE 补充。posts 表针对 user_id、parent_id、category 建立了索引以优化查询。team_members 和 post_likes 使用 UNIQUE 约束防止重复记录。

### **富文本编辑器与 Bilibili 视频嵌入**

帖子编辑器使用 TipTap 富文本编辑器，支持 @提及用户（自动补全弹窗），以及嵌入 Bilibili 视频（通过自定义 BilibiliNode 节点解析 BV/AV/EP 号）。mentionParser.js 负责在提交时从 HTML 中提取 @提及的用户信息。

TipTap没有接入视频、图像等模式，这实际上非常不方便。如果你想要使用我的富文本编辑器，可以将正则匹配换为youtube或者tiktok的匹配模式。另外，你也可以直接在服务器内部检索VedioID，可以直接调用内部的视频文件。
...
另外提一下，我没有打包 `Amiya.mp4` 和 `Amiya2.mp4` 这两个背景视频文件到仓库里，所以你第一次编译会报错，提示你缺失了这两个文件。这是因为它们太大了，GitHub仓库接收不了。如果你需要这两个视频，可以去该项目的GitHub Pages页面下载，或者用自己的视频替代；当然你可以自己重新设计样式，这些都就是没问题的。