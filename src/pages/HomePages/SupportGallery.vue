<template>
  <div class="support-gallery" ref="galleryRef">
    <div class="container">
...
function handleMouseMove(e) {
  ...
  const leftChar = document.querySelector('.support-gallery .char-wrapper--left')
  const rightChar = document.querySelector('.support-gallery .char-wrapper--right')

  if (leftChar) {
    leftChar.style.setProperty('--parallax-x', `${xOffset * 3}px`)
    leftChar.style.setProperty('--parallax-y', `${yOffset * 1.5}px`)
  }
  if (rightChar) {
    rightChar.style.setProperty('--parallax-x', `${xOffset * 1.8}px`)
    rightChar.style.setProperty('--parallax-y', `${yOffset * 0.8}px`)
  }
}
...
<style scoped>
/* ========================================
   支援 Gallery - Overwatch 风格优化
   ======================================== */

.support-gallery {
  height: 100vh;
  background: linear-gradient(135deg, #141005 0%, #1d1608 30%, #2a1f08 60%, #141005 100%);
  color: rgb(0, 0, 0);
  overflow: hidden;
  position: relative;
}
...
.char-wrapper--right img {
  height: 100%;
  width: auto;
  filter: drop-shadow(0 20px 40px rgba(234,179,8,0.3));
}
...
.desc-text {
  ...
  margin-bottom: 2vh;
  text-shadow: 0 0 20px rgba(234,179,8,0.1);
}
...
.hero-card-glow {
  ...
  border-radius: 14px;
  opacity: 0;
  background: radial-gradient(circle at 50% 50%, rgba(234,179,8,0.25), transparent 70%);
...
.hero-item:hover .hero-card img {
  border-color: #facc15;
  box-shadow: 0 0 20px rgba(234,179,8,0.4), 0 8px 30px rgba(234,179,8,0.2);
